...
GPS::GPS()
  : gpsSerial(nullptr), initialized(false), lastUpdateTime(0), lastFixTime(0), homeLat(0), homeLon(0), homeSet(false), errorCount(0), noFixCount(0) {
...
#if ENABLE_GPS == 0
  LOGW("GPS disabled in V2 configuration (pins reallocated to solve conflicts)");
  initialized = false;
  return false;
#elif defined(ESP32)
...
bool GPS::isAccurate() const {
  return currentData.fix && currentData.satellites >= GPS_MIN_SATELLITES && currentData.hdop < GPS_ACCURACY_THRESHOLD;
}

bool GPS::isHealthy() const {
  return initialized && (millis() - lastUpdateTime < 5000) && (millis() - lastFixTime < 30000) && errorCount < 100;
}
...
bool GPS::hasGoodFix() const {
  return currentData.fix && currentData.satellites >= GPS_MIN_SATELLITES && gps.location.age() < 1000;
}
